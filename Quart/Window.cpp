...
namespace Quart
{
	...
	LRESULT Window::WindowProc(HWND hwnd, UINT message,WPARAM wparam,LPARAM lparam)
	{
		...
		case WM_COMMAND:
			{
				if(this->controllers.count(LOWORD(wparam)))
				{
					if(this->controllers[LOWORD(wparam)]->type == Controller::ACCELERATOR)
						this->controllers[LOWORD(wparam)]->MsgHandler(0, lparam);
					else
						this->controllers[LOWORD(wparam)]->MsgHandler(HIWORD(wparam), lparam);
				}
			}
			break;
